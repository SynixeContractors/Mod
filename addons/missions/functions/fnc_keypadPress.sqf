...
if (_keypadInput isEqualTo "Enter") exitWith {
    ...
    [_text, _requiredCode, _keypad, ace_player] call (_keypad getVariable [QGVAR(Keypad_onAttempt), {}]);
    ...
    if (_text isNotEqualTo _requiredCode) exitWith {
        ...
        if (_failedAttempts >= _maxFailedAttempts) exitWith {
            ...
            [_text, _requiredCode, _keypad, ace_player] call (_keypad getVariable [QGVAR(Keypad_onFail), {}]);
            [{
                closeDialog 2;
    ...
    [_requiredCode, _keypad, ace_player] call (_keypad getVariable [QGVAR(Keypad_onComplete), {}]);
...
if (_keypadInput isEqualType 0) then {
    ...
    if (_characterCount > count _requiredCode + 2) exitWith {
        _keypad setVariable [QGVAR(Keypad_Text), "", true];
